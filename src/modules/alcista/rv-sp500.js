...
export async function render(container, { actionsSlot, savedState }) {
  ...
  INDICES_CONFIG.forEach(cfg => {
    const prev = savedState?.indexState?.[cfg.key];
    indexState[cfg.key] = prev
      ? { ...prev, scanning: false }
      : { results: [], scanning: false, done: 0, total: 0 };
  });
  let activeTab = savedState?.activeTab || 'SP500';
  ...
  async function startScan(indexKey) {
    ...
    saveModuleState('alc-rv-sp500', { indexState, activeTab });
  }
  ...
  if (savedState?.indexState) {
    INDICES_CONFIG.forEach(cfg => {
      if (indexState[cfg.key]?.results?.length > 0) {
        const btn = document.getElementById(`sbtn-${cfg.key}`) || container.querySelector(`[data-index="${cfg.key}"] .sc2-btn-scan`);
        const st  = document.querySelector(`#panel-${cfg.key} .sc2-status`);
        if (btn) btn.textContent = '↻ Re-escanear';
        if (st)  st.textContent = `${indexState[cfg.key].results.length} valores · ${indexState[cfg.key].results.filter(r=>r.estado==='ready').length} listos`;
