...
// Acepta futuros (=F) y ETFs.
// Persistencia en localStorage('ethan_watchlist_com_v1')
// Integración con screener de commodities via
// botón + WL que escribe en la misma clave.
// ═══════════════════════════════════════════════

const STORAGE_KEY = 'ethan_watchlist_com_v1';
const VOL_AVG = 11;
...
// Mapa de nombres y emojis para commodities conocidas
const COMMODITY_INFO = {
  'GC=F':{'name':'Oro','emoji':'🥇'}, 'GLD':{'name':'Oro · SPDR Gold Trust','emoji':'🥇'},
  'IAU':{'name':'Oro · iShares','emoji':'🥇'}, 'SI=F':{'name':'Plata','emoji':'🥈'},
  'SLV':{'name':'Plata · iShares','emoji':'🥈'}, 'PL=F':{'name':'Platino','emoji':'⬜'},
  'PA=F':{'name':'Paladio','emoji':'🔘'}, 'PPLT':{'name':'Platino · Sprott','emoji':'⬜'},
  'CL=F':{'name':'Petróleo WTI','emoji':'🛢️'}, 'BZ=F':{'name':'Petróleo Brent','emoji':'🛢️'},
  'USO':{'name':'Petróleo · US Oil Fund','emoji':'🛢️'}, 'UCO':{'name':'Petróleo · ProShares 2x','emoji':'🛢️'},
  'NG=F':{'name':'Gas Natural','emoji':'🔥'}, 'UNG':{'name':'Gas Natural · US Fund','emoji':'🔥'},
  'RB=F':{'name':'Gasolina RBOB','emoji':'⛽'}, 'HO=F':{'name':'Gasóleo','emoji':'🏭'},
  'HG=F':{'name':'Cobre','emoji':'🟤'}, 'CPER':{'name':'Cobre · US ETF','emoji':'🟤'},
  'COPX':{'name':'Cobre · Mineras','emoji':'🟤'}, 'ALI=F':{'name':'Aluminio','emoji':'🔩'},
  'ZNC=F':{'name':'Zinc','emoji':'⚙️'}, 'ZW=F':{'name':'Trigo','emoji':'🌾'},
  'WEAT':{'name':'Trigo · Teucrium','emoji':'🌾'}, 'ZC=F':{'name':'Maíz','emoji':'🌽'},
  'CORN':{'name':'Maíz · Teucrium','emoji':'🌽'}, 'ZS=F':{'name':'Soja','emoji':'🫘'},
  'SOYB':{'name':'Soja · Teucrium','emoji':'🫘'}, 'SB=F':{'name':'Azúcar','emoji':'🍬'},
  'CANE':{'name':'Azúcar · Teucrium','emoji':'🍬'}, 'KC=F':{'name':'Café','emoji':'☕'},
  'JO':{'name':'Café · iPath','emoji':'☕'}, 'CC=F':{'name':'Cacao','emoji':'🍫'},
  'NIB':{'name':'Cacao · iPath','emoji':'🍫'}, 'CT=F':{'name':'Algodón','emoji':'🤍'},
  'LE=F':{'name':'Ganado Bovino','emoji':'🐄'}, 'HE=F':{'name':'Cerdos','emoji':'🐷'},
  'GSG':{'name':'Commodities · iShares','emoji':'🌐'}, 'PDBC':{'name':'Commodities · Invesco','emoji':'🌐'},
  'DJP':{'name':'Commodities · iPath','emoji':'🌐'}, 'COMT':{'name':'Commodities · iShares GSCI','emoji':'🌐'},
};

function getCommodityInfo(ticker) {
  return COMMODITY_INFO[ticker] || { name: ticker, emoji: ticker.includes('=F') ? '📊' : '📈' };
}

// ── Motor técnico (idéntico a rv-watchlist) ──
function ema(arr, p) {
...
function analyze(raw, ticker){
  const{timestamps,opens,highs,lows,closes,vols,currency}=raw;
  const n=closes.length;
  const info=getCommodityInfo(ticker);
  const isFutures=ticker.includes('=F');
  ...
  return{
    name:info.name, emoji:info.emoji, isFutures, currency,
    price:closes[di], score, estado, mensualOk, semanalOk, dailyReady,
...
// ── Helpers visuales (igual que rv-watchlist) ──
...
export async function render(container, { actionsSlot }) {
  ...
  container.innerHTML = `<div id="wl-com-list"></div>`;
  ...
  function renderCard(ticker){
    ...
    if(!d||d.status==='loading') return `
      <div class="wl-card loading" id="wlcard-${ticker.replace('=','-')}">
        <div class="wl-card-main">
          <div>
            <div style="display:flex;align-items:center;gap:8px;">
              <span style="font-size:18px">${info.emoji}</span>
              <div>
                <div class="wl-ticker">${ticker.replace('=F','')}</div>
    ...
    if(d.status==='error') return `
      <div class="wl-card error" id="wlcard-${ticker.replace('=','-')}">
        <div class="wl-card-main">
          <div>
            <div style="display:flex;align-items:center;gap:8px;">
              <span style="font-size:18px">${info.emoji}</span>
    ...
    const fmtP=v=>v!=null&&!isNaN(v)?r.currency==='USD'?'$'+v.toFixed(2):v.toFixed(2)+' '+r.currency:'—';

    return `
      <div class="wl-card ${r.estado}" id="wlcard-${ticker.replace('=','-')}">
            ...
            <div style="display:flex;align-items:center;gap:10px;">
              ...
              <div>
                <div style="display:flex;align-items:center;gap:7px;">
                  <span class="wl-ticker">${ticker.replace('=F','')}</span>
                  <span class="cm-type-badge ${r.isFutures?'cm-type-futures':'cm-type-etf'}">${r.isFutures?'FUTURO':'ETF'}</span>
                </div>
        ...
        <div class="wl-detail ${isOpen?'open':''}" id="wldetail-${ticker.replace('=','-')}">
  ...
  function renderAll(){
    const el=document.getElementById('wl-com-list');
    if(!el)return;
    ...
    const lastUpEl=document.getElementById('wl-com-last-update');
  ...
  function attachCardListeners(){
    document.querySelectorAll('.wl-expandable').forEach(el=>{
      el.addEventListener('click',e=>{
        ...
        const det=document.getElementById(`wldetail-${ticker.replace('=','-')}`);
  ...
  document.getElementById('wl-com-add-btn')?.addEventListener('click',()=>{
    const input=document.getElementById('wl-com-input');
    if(!input?.value.trim())return;
    addTicker(input.value);
    input.value='';
  });
  document.getElementById('wl-com-input')?.addEventListener('keydown',e=>{
    if(e.key==='Enter'){
      e.preventDefault();
      const input=document.getElementById('wl-com-input');
      if(!input?.value.trim())return;
