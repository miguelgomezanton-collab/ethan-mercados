...
// ══════════════════════════════════════════════════
// analyzeAsset BAJISTA
// Score /10: 6 mensuales + 4 semanales
// Estado: ready = M+S+señal diario, diario = M+S, close = score≥7
// ══════════════════════════════════════════════════
function analyzeAsset(raw) {
  ...
  const m_s14  =stoch(M.highs,M.lows,M.closes,14);
  ...
  const w_s14 =stoch(W.highs,W.lows,W.closes,14);
  const w_rsi =rsi(W.closes,14);

  // ── Indicadores diarios ──
  const d_s8=stoch(highs,lows,closes,8);

  // ── Condiciones MENSUAL (6) ──
  const mc_ok=[
    m_ema10[mi]!=null && M.closes[mi]<m_ema10[mi],                          // Precio < EMA10
    m_macd.m[mi]<0 && m_macd.m[mi]<m_macd.sl[mi],                          // MACD↓ <0
    m_s89.k[mi]<m_s89.d[mi] && (mi<1||m_s89.k[mi-1]>=m_s89.d[mi-1]||m_s89.k[mi]<20), // Stoch89↓
    m_rsi[mi]<41,                                                            // RSI<41
    m_s14.k[mi]<30,                                                          // Stoch14<30
    m_s8.k[mi]<m_s8.d[mi] && (mi<1||m_s8.k[mi-1]>=m_s8.d[mi-1]||m_s8.k[mi]<20)  // Stoch8↓
  ];

  // ── Condiciones SEMANAL (4) ──
  const sc_ok=[
    w_macd.m[wi]<0 && w_macd.m[wi]<w_macd.sl[wi],                          // MACD↓ <0
    w_s89.k[wi]<20 && w_s89.k[wi]<w_s89.d[wi],                             // Stoch89↓ <20
    w_s14.k[wi]<20,                                                          // Stoch14<20
    w_rsi[wi]<40                                                             // RSI<40
  ];

  const mensualOk=mc_ok.every(x=>x), semanalOk=sc_ok.every(x=>x);

  // Señal diaria: Stoch(8) corta a la baja
  const s8_cross_down = di>0 && d_s8.k[di]<d_s8.d[di] && d_s8.k[di-1]>=d_s8.d[di-1];

  // Score /10
  const score = mc_ok.filter(x=>x).length + sc_ok.filter(x=>x).length;

  let estado='watching';
  if (mensualOk&&semanalOk&&s8_cross_down) estado='ready';
...
export async function render(container, { actionsSlot, savedState }) {
            ...
            <div class="sc2-filters">
              ...
              <div class="sc2-filter"><label>ESTADO</label>
                <select id="fe-${cfg.key}" class="sc2-sel">
                  ...
                  <option value="ready">🔴 Corto Ready</option>
  ...
  async function startScan(indexKey) {
    ...
    saveModuleState('baj-sp500', { indexState, activeTab });
  }
  ...
  if (savedState?.indexState) {
    INDICES_CONFIG.forEach(cfg => {
      if (indexState[cfg.key]?.results?.length > 0 && cfg.key === activeTab) renderTable(cfg.key);
    });
  }
