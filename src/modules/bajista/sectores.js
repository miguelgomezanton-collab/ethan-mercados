...
function analyzeAsset(raw) {
  ...
  const m_s14=stoch(M.highs,M.lows,M.closes,14), m_s8=stoch(M.highs,M.lows,M.closes,8);
  const m_rsi=rsi(M.closes,14), m_ema10=ema(M.closes,10);
  const w_macd=macd(W.closes), w_s89=stoch(W.highs,W.lows,W.closes,89);
  const w_s14=stoch(W.highs,W.lows,W.closes,14), w_rsi=rsi(W.closes,14);
  const d_s8=stoch(highs,lows,closes,8);

  const mc_ok=[
    m_ema10[mi]!=null && M.closes[mi]<m_ema10[mi],
    m_macd.m[mi]<0 && m_macd.m[mi]<m_macd.sl[mi],
    m_s89.k[mi]<m_s89.d[mi] && (mi<1||m_s89.k[mi-1]>=m_s89.d[mi-1]||m_s89.k[mi]<20),
    m_rsi[mi]<41,
    m_s14.k[mi]<30,
    m_s8.k[mi]<m_s8.d[mi] && (mi<1||m_s8.k[mi-1]>=m_s8.d[mi-1]||m_s8.k[mi]<20)
  ];
  const sc_ok=[
    w_macd.m[wi]<0 && w_macd.m[wi]<w_macd.sl[wi],
    w_s89.k[wi]<20 && w_s89.k[wi]<w_s89.d[wi],
    w_s14.k[wi]<20,
    w_rsi[wi]<40
  ];

  const mensualOk=mc_ok.every(x=>x), semanalOk=sc_ok.every(x=>x);
  const s8_cross=di>0&&d_s8.k[di]<d_s8.d[di]&&d_s8.k[di-1]>=d_s8.d[di-1];
  const score=mc_ok.filter(x=>x).length+sc_ok.filter(x=>x).length;

  let estado='watching';
  if (mensualOk&&semanalOk&&s8_cross) estado='ready';
...
export async function render(container, { actionsSlot, savedState }) {
  ...
  SECTORS_CONFIG.forEach(cfg => {
    const prev = savedState?.sectorState?.[cfg.key];
    sectorState[cfg.key] = prev
      ? { ...prev, scanning: false }
      : { results: [], scanning: false, done: 0, total: 0 };
  });
  let activeTab = savedState?.activeTab || 'XLK';
  let activeView = 'rotacion';
  let rotacionResults = savedState?.rotacionResults || [];
  ...
  async function addToWatchlistLocal(ticker) {
    try {
      const list = (await UserData.get('ethan_watchlist_bajista_v1')) || [];
      if (!list.includes(ticker)) {
        list.push(ticker);
        await UserData.set('ethan_watchlist_bajista_v1', list);
      }
  ...
  function renderTable(sectorKey) {
    ...
    const estadoLabel = { ready:'🔴 CORTO', diario:'⏳ ESPERA DIARIO', close:'🔶 CERCA', watching:'👁 VIGILANDO' };
  ...
  async function startScan(sectorKey) {
    ...
    saveModuleState('baj-sectores', { sectorState, rotacionResults, activeTab });
  }
  ...
  // Restaurar resultados previos
  if (savedState?.sectorState) {
    SECTORS_CONFIG.forEach(cfg => {
      if (sectorState[cfg.key]?.results?.length > 0) {
        const btn = document.getElementById(`sbtn-${cfg.key}`);
        const st  = document.getElementById(`sst-${cfg.key}`);
        if (btn) btn.textContent = '↻ Re-escanear';
        if (st)  st.textContent = `${sectorState[cfg.key].results.length} valores · ${sectorState[cfg.key].results.filter(r=>r.estado==='ready').length} listos`;
        if (cfg.key === activeTab) renderTable(cfg.key);
      }
    });
  }
  if (savedState?.rotacionResults?.length > 0) renderRotacion();
